...
import de.kumpelblase2.remoteentities.helper.world.WorldUtilities;
import org.bukkit.*;
...
class ChunkEntityLoader implements Listener
{
	...
	public void onChunkLoad(ChunkLoadEvent event)
	{
		...
		Bukkit.getScheduler().runTask(RemoteEntities.getImplementation().getPlugin(), new Runnable() {
			public void run()
			{
	 ...
	 * @param inEntity		Entity to spawn
	 * @param inLocation	Location to spawn at
	 * @param inSetupGoals	Whether standard de.kumpelblase2.remoteentities.api.goals should be applied or not
	 * @return				true if it gets queued, false if it could be spawned directly
	 */
